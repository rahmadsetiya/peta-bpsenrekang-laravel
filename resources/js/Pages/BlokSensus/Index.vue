<template>
  <AppLayout title="Blok Sensus">
    <DataTable :items="items.data" :col-count="6" v-model="search">
      <template #actions>
        <Link :href="route('blok-sensus.create')" class="btn btn-primary">
          <i class="fas fa-plus" style="font-size:11px"></i> Tambah
        </Link>
        <a :href="route('blok-sensus.export')" class="btn btn-success">
      ...
      <template #headers>
        <th class="table-th">Kode BS</th>
        <th class="table-th">Nama BS</th>
        <th class="table-th">Desa</th>
        <th class="table-th">Kecamatan</th>
      </template>
      <template #row="{ row }">
        <td class="table-td font-mono text-xs text-gray-600">{{ row.kode_bs }}</td>
        <td class="table-td font-medium">{{ row.nama_bs }}</td>
        <td class="table-td-muted">{{ row.nama_desa }}</td>
        <td class="table-td-muted">{{ row.nama_kecamatan }}</td>
      </template>
      <template #rowActions="{ row }">
        <Link :href="route('blok-sensus.detail', row.id)" class="btn-icon btn-icon-info" title="Detail">
          <i class="fas fa-eye text-xs"></i>
        </Link>
        <Link :href="route('blok-sensus.edit', row.id)" class="btn-icon btn-icon-edit" title="Edit">
      ...
      <div v-if="showImport" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
          <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h3 class="text-base font-semibold text-gray-900">Import Excel Blok Sensus</h3>
...
watch(search, val => {
  ...
  searchTimer = setTimeout(() => {
    router.get(route('blok-sensus.index'), { search: val }, { preserveState: true, replace: true })
  }, 350)
})
