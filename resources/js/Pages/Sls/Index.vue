<template>
  <AppLayout title="SLS">
    <DataTable :items="items.data" :col-count="5" v-model="search">
      <template #actions>
        <Link :href="route('sls.create')" class="btn btn-primary">
          <i class="fas fa-plus" style="font-size:11px"></i> Tambah
        </Link>
        <a :href="route('sls.export')" class="btn btn-success">
      ...
      <template #headers>
        <th class="table-th">Kode SLS</th>
        <th class="table-th">Nama SLS</th>
        <th class="table-th">Nama Desa</th>
      </template>
      <template #row="{ row }">
        <td class="table-td font-mono text-xs text-gray-600">{{ row.kode_sls }}</td>
        <td class="table-td font-medium">{{ row.nama_sls }}</td>
        <td class="table-td-muted">{{ row.nama_desa }}</td>
      </template>
      <template #rowActions="{ row }">
        <Link :href="route('sls.detail', row.id)" class="btn-icon btn-icon-info" title="Detail">
          <i class="fas fa-eye text-xs"></i>
        </Link>
        <Link :href="route('sls.edit', row.id)" class="btn-icon btn-icon-edit" title="Edit">
      ...
      <div v-if="showImport" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
          <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h3 class="text-base font-semibold text-gray-900">Import Excel SLS</h3>
...
watch(search, val => {
  ...
  searchTimer = setTimeout(() => {
    router.get(route('sls.index'), { search: val }, { preserveState: true, replace: true })
  }, 350)
})
